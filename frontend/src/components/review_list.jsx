...
const ReplyTextArea = ({ onChange, onSubmit, onClose, value }) => (
  <>
    ...
    <Form.Item>
      ...
      <Button size='small' onClick={onClose} style={{ margin: '0 5px' }}>
        Cancel
      </Button>
    </Form.Item>
  </>
...
const ReplyButton = (props) => {
  ...
  const sendComment = () => {
    ...
    axios
      .patch('http://127.0.0.1:5000/hostreplies', body)
  ...
  return (
    <>
      {commentArea ? (
        ...
        <>
          <span key='comment-list-reply-to-0' onClick={openCommentArea}>
            Reply to
          </span>
        </>
      )}
...
const showDeleteConfirm = (targetUserId) => {
  confirm({
    ...
    onOk() {
      axios
        .delete('http://127.0.0.1:5000/hostreplies', {
          data: {
            token: localStorage.getItem('token'),
...
const DeleteReplyButton = ({ targetUserId }) => {
  ...
  return (
    <span key='comment-list-reply-to-0' onClick={deleteAction}>
      Delete Reply
    </span>
  );
};
...
export default function ReviewList(props) {
  ...
  const handleReviewSubmit = (e) => {
    axios
      .post('http://127.0.0.1:5000/reviews', e)
      .then((res) => res.data)
      .then((data) => {
        if (data.status.toUpperCase() === 'SUCCESS') {
  ...
  useEffect(() => {
    const requestURL =
      'http://127.0.0.1:5000/reviews?token=' +
  ...
  const WriteReviewSection = () => {
    if (!reviewList.has_ticket) {
      return (
        <h2>
          Sorry you can't post a review on this event, since you have not
          attended it.
        </h2>
      );
    } else {
      if (reviewList.has_comment) {
        ...
        return <WriteReview handleSubmit={handleReviewSubmit} />;
      }
    }
  };
            ...
            <Comment
              actions={
                reviewList.is_host
                  ? item.reply
                    ? null
                    : [<ReplyButton targetUserId={item.reviewedByUserId} />]
                  : null
              }
              ...
              datetime={
                <Tooltip
                  title={moment(item.reviewedOn).format('YYYY-MM-DD HH:mm:ss')}
                >
                ...
                <Comment
                  actions={
                    props.isEventHost
                      ? [
                          <DeleteReplyButton
                            targetUserId={item.reviewedByUserId}
                          />,
                        ]
                      : null
                  }
                  ...
                  datetime={
                    <Tooltip
                      title={moment(item.repliedOn).format('YYYY-MM-DD HH:mm')}
                    >
