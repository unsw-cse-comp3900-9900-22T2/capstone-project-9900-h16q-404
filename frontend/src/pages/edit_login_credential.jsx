...
const EditLoginCredentialForm = () => {
  ...
  const onFinishLoginCredential = (values) => {
    ...
    axios
      .patch('http://127.0.0.1:5000/user/login_credentials', body)
      .then((res) => res.data)
      .then((data) => {
  ...
  return (
    <Form name='edit-login' form={form} onFinish={onFinishLoginCredential}>
          ...
          ({ getFieldValue }) => ({
            validator(_, value) {
              ...
              return Promise.reject(
                new Error('New password can not be same as old password!')
              );
            },
          }),
        ]}
